...
import { Home, Heart, BarChart3, HeartHandshake, Settings, Shield } from "lucide-react";
...
export const Navigation = () => {
  ...
  const links = [
    ...
    { to: '/support', icon: HeartHandshake, label: t('nav.support') },
    ...
    <nav className="sticky top-0 z-50 w-full border-b border-border bg-card/80 backdrop-blur-lg">
      ...
      <div className="md:hidden fixed bottom-0 left-0 right-0 bg-card border-t border-border z-50">
        <div className="flex items-center justify-around py-2">
          {links.filter(l => !l.adminOnly).map((link) => (
            <Button
              key={link.to}
              variant="ghost"
              size="sm"
